<?php 
		session_start();
		function guardar_casino($rut,$dv, $nombre ,$apellido_paterno ,$apellido_materno, $tipo_casino, $correo)
	{
		require 'conexion.php';
		$sql="INSERT INTO casino (rut, dv, nombre, apellido_pat, apellido_mat, tipo_casino, correo) VALUES (?,?,?,?,?,?,?)";
		...
		$smt -> bindparam(6,$tipo_casino);
		$smt -> bindparam(7,$correo);
		$smt -> execute();
		$conn = null;


		header("Location: crear_usuario_casino.php?RUT=".urlencode($rut));
	...
	$tipo_casino=$_REQUEST['tipo_casino'];
...
$veri = $mysqli->query("SELECT rut FROM casino WHERE rut = $rut");
...
if ($veri->num_rows == 1) {
     ...
     guardar_casino($rut,$dv, $nombre ,$apellido_paterno ,$apellido_materno, $tipo_casino, $correo);
 }
